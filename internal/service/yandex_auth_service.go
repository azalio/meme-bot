...
func (s *YandexAuthServiceImpl) GetIAMToken(ctx context.Context) (string, error) {
	...
	s.logger.Info("No token found, refreshing")
...
func (s *YandexAuthServiceImpl) RefreshIAMToken(ctx context.Context, oauthToken string) (string, error) {
	...
	s.logger.Info("Request body: %s", string(requestBodyJSON))
	...
	s.logger.Info("Request headers: %v", req.Header)

	// Выполняем запрос
	s.logger.Info("Отправляем запрос на получение токена")
	client := http.Client{}
	...
	s.logger.Info("Response status: %d", resp.StatusCode)
	s.logger.Info("Response body: %s", string(bodyBytes))
	...
	s.logger.Info("IAM token получен")
...
func (s *YandexAuthServiceImpl) refreshToken(ctx context.Context) (string, error) {
	s.logger.Info("Пробуем получить новый токен")
	s.mu.Lock()
	defer s.mu.Unlock()
